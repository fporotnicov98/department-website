...
namespace DepartmentWebApi.Services
{
    ...
    public class UserService : IUserService
    {
        ...
        public string Authenticate(string email, string password)
        {
            ...
            {
                byte[] hashPassword;
                using (Argon2i argon2 = new Argon2i(Encoding.UTF8.GetBytes(password)))
                {
                    argon2.Salt = Encoding.UTF8.GetBytes(_appSettings.Salt);
                    argon2.DegreeOfParallelism = 16;
                    argon2.MemorySize = 8192;
                    argon2.Iterations = 40;
                    hashPassword = argon2.GetBytes(512);
                }
        ...
        public bool Registration(UsersWithoutId user)
        {
            ...
            {
                byte[] hashPassword;
                using (Argon2i argon2 = new Argon2i(Encoding.UTF8.GetBytes(user.Password)))
                {
                    argon2.Salt = Encoding.UTF8.GetBytes(_appSettings.Salt);
                    argon2.DegreeOfParallelism = 16;
                    argon2.MemorySize = 8192;
